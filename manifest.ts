...
class GhidraBridgeHandlers {
  constructor(private readonly endpoint: string) {}

  async handleGhidraBridge(args: ToolArgs): Promise<TextToolResponse> {
    ...
    try {
      switch (action) {
        ...
        case 'open_project': {
          ...
          const { status, data } = await requestBridge(this.endpoint, '/project/open', 'POST', { binaryPath });
        ...
        case 'search_strings': {
          ...
          const { status, data } = await requestBridge(this.endpoint, '/strings', 'POST', {
      ...
      return toErrorResponse('ghidra_bridge', error, { action, endpoint: this.endpoint });
    }
  }
}
...
const DEP_KEY = 'ghidraBridgeHandlers';
const DOMAIN = 'ghidra-bridge';
...
function bind(methodName: string) {
  return (deps: ToolHandlerDeps) => async (args: ToolArgs) => {
    ...
    if (typeof method !== 'function') {
      throw new Error(`Missing ghidra handler method: ${methodName}`);
    }
    return method(args ?? {});
  };
}
...
const domainManifest: DomainManifest = {
  ...
  profiles: ['full', 'reverse'],
    ...
    return new GhidraBridgeHandlers(endpoint);
  },
  registrations: [
    {
      tool: tools[0]!,
      domain: DOMAIN,
      bind: bind('handleGhidraBridge'),
    },
  ],
};
...
const plugin: PluginContract = {
  manifest: {
    ...
    version: 1,
    id: 'io.github.vmoranv.ghidra-bridge',
    name: 'Ghidra Bridge',
    pluginVersion: '0.1.0',
    entry: 'manifest.js',
    description: 'Atomic Ghidra bridge plugin.',
    compatibleCore: '>=0.1.0',
    permissions: {
      network: { allowHosts: ['127.0.0.1', 'localhost', '::1'] },
      process: { allowCommands: [] },
      filesystem: { readRoots: [], writeRoots: [] },
      toolExecution: { allowTools: ['ghidra_bridge'] },
    },
    activation: {
      onStartup: false,
      profiles: ['full', 'reverse'],
    ...
    contributes: {
      ...
      workflows: [],
      configDefaults: {
        'plugins.ghidra-bridge.enabled': true,
      },
      metrics: ['ghidra_bridge_calls_total'],
    },
  },
  ...
  onRegister(ctx: PluginLifecycleContext): void {
    ...
    ctx.registerMetric('ghidra_bridge_calls_total');
  },
};
